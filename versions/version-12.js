...
function renderRoleMarkdownNote(markdown = "", body) {
  ...
  roleNote.textContent = lines[0] || "";
  sectionNote.textContent = lines.slice(1).join(" ") || "";
  body.append(roleNote, sectionNote);
}
...
function renderProfileLinks(item, titleTarget, titleText) {
  ...
  appendLink("LinkedIn", item.meta?.linkedin);
  appendLink("Google Scholar", item.meta?.scholar);
...
function renderIdentityRole(item, body) {
  ...
  statement.className = "identity-statement";
  statement.textContent = getLocalizedMarkdown(item);

  figure.className = "identity-detail-portrait";
  image.src = item.meta?.image || "../assets/identity-portrait.png";
...
function renderRoleItems() {
  ...
  if (visualTarget) {
    visualTarget.replaceChildren(
      ...topLevelGalleryItems.slice(0, 6).map((item) =>
        createRoleItemButton(getLocalizedTitle(item), () => renderMarkdownReader(item.id, true), {
          kicker: item.meta?.year || "gallery",
